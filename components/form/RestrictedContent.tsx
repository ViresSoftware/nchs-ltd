...
export default function RestrictedContent({ children }: { children?: React.ReactNode }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(`https://nchsltdadmin.com/wp-json/contact-form-7/v1/contact-forms/55/feedback`, {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <>
      ...
      {!isVerified && (
        <div
          className="fixed inset-0 z-[9999] bg-black/70 backdrop-blur-sm text-white flex items-center justify-center"
